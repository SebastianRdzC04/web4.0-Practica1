...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        datosPhp.forEach(categoria => {
            if (categoria.total > valorMaximo) valorMaximo = categoria.total;
        });
        ...
        datosPhp.forEach((categoria, i) => {
            ...
            const anchoBarra = anchoCategorias * 0.7; // 70% del espacio disponible
            const espacioTotal = anchoCategorias * 0.7;
            const inicioGrupo = centroCategoriaX - (espacioTotal / 2);

            // Dibujar etiqueta de total sobre la barra
            ctx.fillStyle = '#475569';
            ctx.textAlign = 'center';
            ctx.font = 'bold 14px Inter, sans-serif';
            ctx.fillText(`Total: ${categoria.total}`, centroCategoriaX, areaGrafica.y - 15);

            // Dibujar barra de fondo (total)
            const alturaTotalBarra = categoria.total * escalaY;
            const yTotal = areaGrafica.y + areaGrafica.alto - alturaTotalBarra;

            // Barra de fondo con gradiente y borde redondeado
            const totalGradient = ctx.createLinearGradient(inicioGrupo, yTotal, inicioGrupo, areaGrafica.y + areaGrafica.alto);
            totalGradient.addColorStop(0, 'rgba(226, 232, 240, 0.7)');
            totalGradient.addColorStop(1, 'rgba(226, 232, 240, 0.3)');

            // Dibujar barra redondeada para el total
            roundedRect(ctx, inicioGrupo, yTotal, anchoBarra, alturaTotalBarra, 6, totalGradient);

            // Altura acumulada para apilar las barras internas
            let alturaAcumulada = 0;

            // Dibujar cada barra del grupo (hombres y mujeres)
            categoria.data.forEach((item, j) => {
                const alturaBarra = item.data * escalaY;
                const y = areaGrafica.y + areaGrafica.alto - alturaAcumulada - alturaBarra;

                // Crear gradiente para la barra
                const barGradient = ctx.createLinearGradient(inicioGrupo, y, inicioGrupo, y + alturaBarra);
                ...
                // Dibujar barra interna redondeada solo en la parte superior
                const isLast = j === categoria.data.length - 1;
                roundedRectTop(ctx, inicioGrupo, y, anchoBarra, alturaBarra, isLast ? 6 : 0, barGradient);

                // Etiquetar valor dentro de la barra si hay espacio
                if (alturaBarra > 25) {
                    ctx.fillStyle = 'white';
                    ctx.font = 'bold 12px Inter, sans-serif';
                    ctx.textAlign = 'center';
                    ctx.fillText(item.data.toString(), inicioGrupo + anchoBarra/2, y + alturaBarra/2 + 5);
                }

                alturaAcumulada += alturaBarra;
            });
        ...
        // Función para dibujar rectángulos con bordes redondeados solo en la parte superior
        function roundedRectTop(ctx, x, y, width, height, radius, fillStyle) {
            ctx.beginPath();
            ctx.moveTo(x + radius, y);
            ctx.lineTo(x + width - radius, y);
            ctx.quadraticCurveTo(x + width, y, x + width, y + radius);
